...
A fonte de tr\'afego nesse experimento \'e somente de uma, a qual realiza uma conex\~ao do Soldado 3(fonte) ao Soldado 1(destino), onde inicialmente vai passar pelo Soldado 2 essa comunica\c{c}\~ao, pois os Soldados 1 e 3 n\~ao est\~ao pr\'oximos a fim de criarem uma rota direta.
...
\begin{figure}[H]
	\centering
	\subfigure[Quantidade de pacotes perdidos]{
		\includegraphics[scale=0.5]{lost.eps}
	}\label{subfig:exp1Lost}
	\subfigure[Atraso m\'edio (s)]{
		\includegraphics[scale=0.5]{aodvLate.eps}
	}\label{subfig:exp1Late}
	\subfigure[Quantidade de \textit{bytes} de roteamento]{
		\includegraphics[scale=0.5]{byte.eps}
	}\label{subfig:exp1Byte}
	\subfigure[Quantidade de pacotes de roteamento]{
		\includegraphics[scale=0.5]{pkts.eps}
